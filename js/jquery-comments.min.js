...
if(n.options.enableUpvoting&&(i+=e.upvoteCount,a+=t.upvoteCount),a!=i)return a-i
...
var r=o.find(".textarea")
r.attr("data-comment",a.id),r.append(this.getFormattedCommentContent(a,!0)),this.moveCursorToEnd(r)},showDroppableOverlay:function(e){this.options.enableAttachments&&(this.$el.find(".droppable-overlay").css("top",this.$el[0].scrollTop),this.$el.find(".droppable-overlay").show(),this.$el.addClass("drag-ongoing"))},handleDragEnter:function(t){var n=e(t.currentTarget).data("dnd-count")||0
...
h.append(D)}var I=e("<div/>",{class:"actions"}),$=e("<button/>",{class:"mdl-button action reply",type:"button",text:this.options.textFormatter(this.options.replyText)}),E=e('<i class="material-icons">thumb_up</i>')
this.options.upvoteIconURL.length&&(E.css("background-image",'url("'+this.options.upvoteIconURL+'")'),E.addClass("image"))
var L=this.createUpvoteElement(t),A=e('<i class="material-icons">thumb_down</i>')
this.options.downvoteIconURL.length&&(A.css("background-image",'url("'+this.options.downvoteIconURL+'")'),A.addClass("image"))
var B=this.createDownvoteElement(t)
if(this.options.enableReplying&&I.append($),t.createdByCurrentUser||this.options.currentUserIsAdmin)if(u&&this.isAllowedToDelete(t.id)){var M=e("<button/>",{class:"mdl-button action delete enabled",text:this.options.textFormatter(this.options.deleteText)})
I.append(M)}else if(!u&&this.options.enableEditing){var F=e("<button/>",{class:"mdl-button action edit",text:this.options.textFormatter(this.options.editText)})
I.append(F)}this.options.enableDownvoting&&I.append(B),this.options.enableUpvoting&&I.append(L),m.append(h),m.append(I)
var _=e("<div/>",{class:"comment-info-wrapper"})
return _.append(i).append(r).append(a),n.append(_).append(m),n},createUpvoteElement:function(t){var n=e('<i class="material-icons">thumb_up</i>')
...
t.css("height",n+"em"),i.css("height",n+"em")}(r),r++,a=t[0].scrollHeight>t.outerHeight(),o=0!=this.options.textareaMaxRows&&r>this.options.textareaMaxRows,t.text().length>0?t.css({display:"flex","align-items":"center",width:"auto"}):t.css({display:"table-cell","vertical-align":"middle",width:"100em"})}while(a&&!o)},clearTextarea:function(e){e.empty().trigger("input")},getTextareaContent:function(t,n){var i=t.clone()
...
return o=this.normalizeSpaces(o)},getFormattedCommentContent:function(e,t){var n=this.escape(e.content)
