...
i[0]&&(i[0].size<=2048e3?this.uploadAttachments(i,a):window.alert("File size should be maximum 2MB."))},upvoteComment:function(t){var n,i=this,a=e(t.currentTarget).parents("li.comment").first(),o=a.data().model,r=o.upvoteCount
...
for(var e=0;e<I;e++)$=3*E[e],D.fillStyle="#2196F1",D.fillRect(B,F-$,A-3,$),B+=A+1}}var i=e("<div/>",{class:"comment-wrapper"}),a=this.createProfilePictureElement(t.profilePictureURL),o=e("<div/>",{class:"time",text:this.options.timeFormatter(t.created),"data-original":t.created}),r=t.createdByCurrentUser?this.options.textFormatter(this.options.youText):t.fullname,s=e("<div/>",{class:"name"})
...
l.html(y)
var x=e("<canvas/>",{class:"canvas-"+t.id})
l.prepend(x)
var T=y.get(0),R=x.get(0)
T.crossOrigin="anonymous"
var k=this.options.audioContext.createMediaElementSource(T),U=this.options.audioContext.createAnalyser()
R.width=window.innerWidth,R.height=window.innerHeight
var D=R.getContext("2d")
k.connect(U),U.connect(this.options.audioContext.destination),U.fftSize=256
var $,I=U.frequencyBinCount,E=new Uint8Array(I),L=R.width,F=R.height,A=L/I*6,B=0,M=!1
y.bind("play",function(){M=!1,n()}),y.bind("ended",function(){setTimeout(function(){M=!0},1e3)})}else{var H=["archive","audio","code","excel","image","movie","pdf","photo","picture","powerpoint","sound","video","word","zip"],O="fa fa-file-o"
H.indexOf(g)>0?O="fa fa-file-"+g+"-o":H.indexOf(v)>0&&(O="fa fa-file-"+v+"-o")
var P=t.fileName,S=e("<div/>",{class:"attachment-wrapper",text:P}),j=e("<i/>",{class:O})
this.options.fileIconURL.length&&(j.css("background-image",'url("'+this.options.fileIconURL+'")'),j.addClass("image")),S.append(j),l.prepend(S)}u.html(l)}else if(t.content.length>300){var N=t.content.substr(300,t.content.length-1)
if(N.length>50&&N.indexOf("")>-1){var z=this.getFormattedCommentContent(t.content.substr(0,300+N.indexOf(" ")))+'<span class="moreellipses">...</span><span class="morecontent"><span class="morelink readmore-'+t.id+'">Show more >></span></span>'
u.html(z)}else u.html(this.getFormattedCommentContent(t.content))
var q=e(".comments-wrapper")
q.off("click","span.readmore-"+t.id),q.on("click","span.readmore-"+t.id,_.bind(function(n){var i
return e(n.currentTarget).hasClass("less")?(i=this.getFormattedCommentContent(t.content.substr(0,300+N.indexOf(" ")))+'<span class="moreellipses">...</span><span class="morecontent"><span class="morelink readmore-'+t.id+'">Show more >></span></span>',u.html(i)):(i=this.getFormattedCommentContent(t.content)+'<span class="less morelink readmore-'+t.id+'">Show less >></span>',u.html(i)),!1},this))}else u.html(this.getFormattedCommentContent(t.content))
if(t.modified&&t.modified!=t.created){var W=this.options.timeFormatter(t.modified),K=e("<time/>",{class:"edited",text:this.options.textFormatter(this.options.editedText)+" "+W,"data-original":t.modified})
u.append(K)}var V=e("<div/>",{class:"actions"}),Z=e("<button/>",{class:"mdl-button action reply",type:"button",text:this.options.textFormatter(this.options.replyText)}),J=e('<i class="material-icons">thumb_up</i>')
this.options.upvoteIconURL.length&&(J.css("background-image",'url("'+this.options.upvoteIconURL+'")'),J.addClass("image"))
var Q=this.createUpvoteElement(t),Y=e('<i class="material-icons">thumb_down</i>')
this.options.downvoteIconURL.length&&(Y.css("background-image",'url("'+this.options.downvoteIconURL+'")'),Y.addClass("image"))
var G=this.createDownvoteElement(t)
if(this.options.enableReplying&&V.append(Z),t.createdByCurrentUser||this.options.currentUserIsAdmin)if(f&&this.isAllowedToDelete(t.id)){var X=e("<button/>",{class:"mdl-button action delete enabled",text:this.options.textFormatter(this.options.deleteText)})
V.append(X)}else if(!f&&this.options.enableEditing){var ee=e("<button/>",{class:"mdl-button action edit",text:this.options.textFormatter(this.options.editText)})
V.append(ee)}this.options.enableDownvoting&&V.append(G),this.options.enableUpvoting&&V.append(Q),h.append(u),h.append(V)
var te=e("<div/>",{class:"comment-info-wrapper"})
return te.append(a).append(s).append(o),i.append(te).append(h),i},createUpvoteElement:function(t){var n=e('<i class="material-icons">thumb_up</i>')
