...
if(n=i.attr("data-comment")){var c,p=d!=this.commentsById[n].content
...
this.uploadAttachments(i,a)},upvoteComment:function(t){var n,i=this,a=e(t.currentTarget).parents("li.comment").first(),o=a.data().model,r=o.upvoteCount
...
var h=e("<button/>",{class:"mdl-button delete",text:this.options.textFormatter(this.options.deleteText)}).css("color",this.options.deleteButtonColor)
d.append(h),this.isAllowedToDelete(n)&&h.addClass("enabled")}else if(m=this.options.textFormatter(this.options.sendText),this.options.enableAttachments){var u=e("<button/>",{class:"mdl-button enabled upload"}),f=e('<i class="material-icons">attachment</i>'),g=e("<input/>",{type:"file","data-role":"none"})
e.browser.mobile||g.attr("multiple","multiple"),this.options.uploadIconURL.length&&(f.css("background-image",'url("'+this.options.uploadIconURL+'")'),f.addClass("image")),u.append(f).append(g),d.append(u.clone()),i&&l.append(u.clone().addClass("inline-button"))}var v=n?"update":"send",C=e("<button/>",{class:v+" mdl-button save",text:m}).css("color",this.options.highlightColor)
...
c.prepend(y)}}return this.options.enablePinging&&(c.textcomplete([{match:/(^|\s)@(([a-zäöüß]|\s)*)$/im,search:function(t,n){t=a.normalizeSpaces(t)
...
return n.append(a),null==t.parent&&n.append(i),n},createCommentWrapperElement:function(t){var n=e("<div/>",{class:"comment-wrapper"}),i=this.createProfilePictureElement(t.profilePictureURL),a=e("<div/>",{class:"time",text:this.options.timeFormatter(t.created),"data-original":t.created}),o=t.createdByCurrentUser?this.options.textFormatter(this.options.youText):t.fullname,r=e("<div/>",{class:"name"})
...
r.append(p)}var m=e("<div/>",{class:"wrapper"}),h=e("<div/>",{class:"content"}),u=(e("<div/>",{class:"toggle-content"}),void 0!=t.fileURL)
...
s.html(b)}else if("audio"==g){var w=e("<audio/>",{src:t.fileURL,type:t.fileMimeType,controls:"controls"})
s.html(w)}else{var y=["archive","audio","code","excel","image","movie","pdf","photo","picture","powerpoint","sound","video","word","zip"],x="fa fa-file-o"
y.indexOf(f)>0?x="fa fa-file-"+f+"-o":y.indexOf(g)>0&&(x="fa fa-file-"+g+"-o")
var T=e("<i/>",{class:x})
...
k=k.split("?")[0],k=decodeURIComponent(k),s.text(k),s.prepend(T)}h.html(s)}else if(t.content.length>300){var U=t.content.substr(300,t.content.length-1)
if(U.length>50&&U.indexOf("")>-1){var D=this.getFormattedCommentContent(t.content.substr(0,300+U.indexOf(" ")))+'<span class="moreellipses">...</span><span class="morecontent"><span class="morelink readmore-'+t.id+'">Show more >></span></span>'
h.html(D)}else h.html(this.getFormattedCommentContent(t.content))
var I=e(".comments-wrapper")
I.off("click","span.readmore-"+t.id),I.on("click","span.readmore-"+t.id,_.bind(function(n){var i
return e(n.currentTarget).hasClass("less")?(i=this.getFormattedCommentContent(t.content.substr(0,300+U.indexOf(" ")))+'<span class="moreellipses">...</span><span class="morecontent"><span class="morelink readmore-'+t.id+'">Show more >></span></span>',h.html(i)):(i=this.getFormattedCommentContent(t.content)+'<span class="less morelink readmore-'+t.id+'">Show less >></span>',h.html(i)),!1},this))}else h.html(this.getFormattedCommentContent(t.content))
if(t.modified&&t.modified!=t.created){var $=this.options.timeFormatter(t.modified),E=e("<time/>",{class:"edited",text:this.options.textFormatter(this.options.editedText)+" "+$,"data-original":t.modified})
h.append(E)}var L=e("<div/>",{class:"actions"}),A=e("<button/>",{class:"mdl-button action reply",type:"button",text:this.options.textFormatter(this.options.replyText)}),F=e('<i class="material-icons">thumb_up</i>')
this.options.upvoteIconURL.length&&(F.css("background-image",'url("'+this.options.upvoteIconURL+'")'),F.addClass("image"))
var B=this.createUpvoteElement(t),M=e('<i class="material-icons">thumb_down</i>')
this.options.downvoteIconURL.length&&(M.css("background-image",'url("'+this.options.downvoteIconURL+'")'),M.addClass("image"))
var H=this.createDownvoteElement(t)
if(this.options.enableReplying&&L.append(A),t.createdByCurrentUser||this.options.currentUserIsAdmin)if(u&&this.isAllowedToDelete(t.id)){var P=e("<button/>",{class:"mdl-button action delete enabled",text:this.options.textFormatter(this.options.deleteText)})
L.append(P)}else if(!u&&this.options.enableEditing){var O=e("<button/>",{class:"mdl-button action edit",text:this.options.textFormatter(this.options.editText)})
L.append(O)}this.options.enableDownvoting&&L.append(H),this.options.enableUpvoting&&L.append(B),m.append(h),m.append(L)
var S=e("<div/>",{class:"comment-info-wrapper"})
return S.append(i).append(r).append(a),n.append(S).append(m),n},createUpvoteElement:function(t){var n=e('<i class="material-icons">thumb_up</i>')
...
do{!function(e){var n=5+1.5*(e-1)
t.css("height",n+"em"),i.css("height",n+"em")}(r),r++,a=t[0].scrollHeight>t.outerHeight(),o=0!=this.options.textareaMaxRows&&r>this.options.textareaMaxRows}while(a&&!o)},clearTextarea:function(e){e.empty().trigger("input")},getTextareaContent:function(t,n){var i=t.clone()
...
a.moveToElementText(t),a.collapse(!1),a.select()}t.focus()},escape:function(t){return e("<pre/>").text(this.normalizeSpaces(t)).html()},normalizeSpaces:function(e){return e.replace(new RegExp(" ","g")," ")},after:function(e,t){var n=this
...
if(-1!=t.indexOf("#")){var i=function(e){return n.createTagElement("#"+e,"hashtag",e)[0].outerHTML},a=/(^|\s)#([a-zäöüß\d-_]+)/gim
